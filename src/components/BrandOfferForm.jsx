...
const BrandOfferForm = () => {
  ...
  const { user } = useContext(MyContext); // Changed to match list component
  ...
  const handleFileChange = (e) => {
    ...
    setFormData((prev) => ({ ...prev, offerImage: file }));
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if user exists and has brandId
    if (!user || !user.brandId) {
      toast.error("You must be logged in as a Brand to submit an offer.");
      setIsSubmitting(false);
      return;
    }
    ...
    try {
      ...
      if (formData.offerImage) {
        imageUrl = await uploadImage(formData.offerImage);
      }

      const id = `OID${Date.now()}`;

      // FIXED: Match the field names with the list component
      await setDoc(doc(db, "offers", id), {
        ...
        createdAt: new Date(),
        status: "pending",
        brandId: user.brandId, // FIXED: Changed from brandId to vendorId
        offerId: id,
      });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <section className="mb-8">
        <h1 className="text-2xl font-bold mb-2">Create Your Offer</h1>
